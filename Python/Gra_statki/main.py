...
def getInputFromComputer(boardWidth, boardHeight, battleshipWasHit, lastShot, game):
    if battleshipWasHit:
        ...
        while (lastShot[0] + randomDirX < 1) or (lastShot[0] + randomDirX > boardWidth) or (lastShot[1] + randomDirY < 1) or (lastShot[1] + randomDirY > boardWidth) or ((lastShot[0] + randomDirX, lastShot[1] + randomDirY) in game.allShots):
        ...
        return (random.randrange(1, boardWidth+1), random.randrange(1, boardWidth+1))
...
def playerVsComputer():
    ...
    while 1:
        ...
        while doWhileCondition and not game2IsOver:
            numberOfShips = len(game2.battleships)
            myShot2 = getInputFromComputer(boardWidth, boardHeight, game2IsHit, computerLastHit, game2)
