...
ALTER TABLE rejsy ADD czy_usuniety int;
ALTER TABLE rejsy ADD kapitan_rejsu int;
...
INSERT INTO osoby_rejsu (rejs, pasazer) VALUES (5, 1);
...
INSERT INTO bilety (pasazer, rejs, cena_biletu, miejsce_w_samolocie, klasa, pierwszenstwo) VALUES (1, 5, 170, '13A', 'ekonomiczna', 1);

DELIMITER //
CREATE FUNCTION funkcja1(lp_samolotu INT, lp_rejsu INT)
RETURNS BOOLEAN
MODIFIES SQL DATA
READS SQL DATA
BEGIN
	UPDATE samoloty SET czy_usuniety_z_floty=1 WHERE lp=lp_samolotu;
    UPDATE rejsy SET czy_usuniety=1 WHERE lp = lp_rejsu;
	RETURN row_count() > 0;
END; //
DELIMITER ;
...
DELIMITER //
CREATE FUNCTION funkcja2(lp_zaloga INT, lp_rejsu INT)
RETURNS BOOLEAN
MODIFIES SQL DATA
READS SQL DATA
BEGIN
	IF(SELECT pilot FROM zaloga WHERE lp = lp_zaloga = 1) THEN
		IF(SELECT lp_rejsu FROM rejsy WHERE lp = lp_rejsu) IS NOT NULL THEN
			UPDATE rejsy SET kapitan_rejsu=lp_zaloga WHERE lp = lp_rejsu;
        END IF;
    END IF;
	RETURN row_count() > 0;
END; //
...
DELIMITER //
CREATE FUNCTION funkcja3(lp_pasazera INT, klasa_pasazera VARCHAR(50))
RETURNS BOOLEAN
MODIFIES SQL DATA
READS SQL DATA
BEGIN
	IF(SELECT lp_pasazera FROM pasazerowie WHERE lp = lp_pasazera) IS NOT NULL THEN
			UPDATE bilety SET klasa = klasa_pasazera WHERE pasazer = lp_pasazera;
	END IF;
	RETURN row_count() > 0;
END; //
DELIMITER ;

SELECT funkcja3(3, "pierwsza");

DELIMITER //
CREATE FUNCTION funkcja4(lp_rejsu INT)
...
BEGIN
	DECLARE wynik INT;
	IF(SELECT lp_rejsu FROM rejsy WHERE lp = lp_rejsu) IS NOT NULL THEN
			SELECT SUM(cena_biletu) FROM bilety WHERE rejs = lp_rejsu INTO wynik;
	END IF;
	RETURN wynik;
END; //
DELIMITER ;
...
DELIMITER //
CREATE FUNCTION funkcja5(lp_rejsu INT)
RETURNS VARCHAR(200) deterministic
MODIFIES SQL DATA
READS SQL DATA
BEGIN
	DECLARE wynik1 VARCHAR(100);
	DECLARE wynik2 VARCHAR(100);
	IF(SELECT lp_rejsu FROM rejsy WHERE lp = lp_rejsu) IS NOT NULL THEN
			SELECT GROUP_CONCAT(pasazer) as "wszyscy pasazerowie rejsu" FROM osoby_rejsu WHERE rejs = lp_rejsu AND pasazer IS NOT NULL GROUP BY rejs INTO wynik1;
            SELECT GROUP_CONCAT(DISTINCT klasa) as "klasy podróży wybranego lotu" FROM bilety WHERE rejs = lp_rejsu GROUP BY rejs INTO wynik2;
	END IF;
    RETURN CONCAT_WS('            ', wynik1, wynik2);
END; //
DELIMITER ;
