...
INSERT INTO rejsy (miejsce_odlotu, miejsce_przylotu, data_odlotu, godzina_odlotu, czas_lotu, samolot, linia_lotnicza) VALUES ('Kostaryka', 'Melbourne', '2020-11-22', '11:00', '10godz', 2, 'FlyWell Airlines');
...
BEGIN
    SELECT samolot, count(*) AS ilosc_rejsow FROM rejsy GROUP BY samolot HAVING ilosc_rejsow > 1;
END;//
DELIMITER ;
...
BEGIN
    INSERT INTO pasazerowie (imie, nazwisko, nr_paszportu, telefon, email) VALUES ('Bartek', 'Kochanowski', '123532545', '352523173', 'bratooo@email.com');
	INSERT INTO pasazerowie (imie, nazwisko, nr_paszportu, telefon, email) VALUES ('Wincenty', 'Turowicz', '572384724', '523234527', 'wacekturowicz@outlook.com');
	INSERT INTO pasazerowie (imie, nazwisko, nr_paszportu, telefon, email) VALUES ('Julia', 'Koczyń', '452045829', '324324854', 'juliakoczyn@hotmail.com');
END;//
DELIMITER ;

CREATE VIEW widok4 AS SELECT * FROM bilety WHERE klasa = 'pierwsza';
 
CREATE VIEW widok5 AS SELECT concat_ws(' ', firma, model) as samolot, miejsce_odlotu, miejsce_przylotu
from rejsy r
join samoloty s on r.samolot = s.lp
join rodzaj_samolotow rs on s.rodzaj_samolotu = rs.lp;
